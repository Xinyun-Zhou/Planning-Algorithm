...
################################################################################
    ...
    def make_control_knowledge_variables(self, horizon: int) -> None:
        ...
        """ *** YOUR CODE HERE *** """
        self.objects = set()
        self.package_at_destination_at_t = {}
        self.vehicle_over_capacity_at_t = {}
        self.package_at_origin_at_t = {}

        for t in range(horizon):
            for package in self.objects:
                # Variable for the rule "package cannot leave once at destination"
                self.package_at_destination_at_t[(package, t)] = self.new_cnf_code(t,
                                                                                   f"package_at_destination({package})",
                                                                                   package)

                # Variable for the rule "package should not return to its original location"
                self.package_at_origin_at_t[(package, t)] = self.new_cnf_code(t, f"package_at_origin({package})",
                                                                              package)

            for vehicle in self.objects:
                # Variable for the rule "vehicle cannot transport more than its capacity"
                self.vehicle_over_capacity_at_t[(vehicle, t)] = self.new_cnf_code(t,
                                                                                  f"vehicle_over_capacity({vehicle})",
                                                                                  vehicle)
    ...
    def make_control_knowledge(self, horizon: int) -> None:
        ...
        for t in range(horizon):
            for package in self.objects:
                # If a package is at its destination at time t, it cannot leave at time t+1
                if t < horizon - 1:
                    self.add_clause(
                        [self.package_at_destination_at_t[(package, t)],
                         -self.package_at_destination_at_t[(package, t + 1)]],
                        "control"
                    )

                # A package should not return to its original location after being moved
                if t > 0:
                    self.add_clause(
                        [-self.package_at_origin_at_t[(package, t - 1)], -self.package_at_origin_at_t[(package, t)]],
                        "control"
                    )

            for vehicle in self.objects:
                # A vehicle cannot transport more packages than its capacity allows
                self.add_clause(
                    [-self.vehicle_over_capacity_at_t[(vehicle, t)]],
                    "control"
                )
